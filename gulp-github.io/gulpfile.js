...
// const uglify = require('gulp-uglify'); //普通压缩js
const javascriptObfuscator = require('gulp-javascript-obfuscator'); // 混淆压缩
...
let paths = {
  ...
  images: {
    app: 'app/images/**/*.{jpg,jpeg,png,gif,svg,ico}',
    dist: `${Dir}/images`
  }
}
...
gulp.task('demos', function () {
  return gulp.src(paths.demos.app)
    .pipe(gulp.dest(paths.demos.dist))
    .pipe(browserSync.stream());
});

//拷贝audio
gulp.task('audio', function () {
  return gulp.src(paths.audio.app)
...
gulp.task('copy', function () {
  return gulp.src([
      ...
      'app/libs/jquery.easing.1.3.js',
      'app/libs/jquery.fullPage.min.js',
      'app/libs/jquery.isotope.js',
      'app/libs/bootstrap.min.js',
      'app/libs/tagcanvas.min.js'
    ])
...
gulp.task('watchs', function () {
  ...
  gulp.watch(paths.demos.app, gulp.series('demos'));
})
...
gulp.task('build', gulp.series(
  clean,
  gulp.parallel('server', 'watchs', 'html', 'css', 'js', 'copy', 'img', 'font', 'audio', 'demos')
))

gulp.task('buildPro', gulp.series(
  clean,
  gulp.parallel('html', 'css', 'js', 'copy', 'img', 'font', 'audio', 'demos')
))
